		...
		for root, subFolders, files in os.walk(self.tiles_directory):
			for tile_name in files:
				...
				if large_tile:
					...
					self.tiles_paths.append(tile_path)
	...
	while True:
		try:
			...
			if img_coords == EOQ_VALUE:
				...
				tablita[str(img_coords)] = best_fit_tile_index

		except KeyboardInterrupt:
			pass

	mosaic.save(mosaic.pathito)
	with open(mosaic.pathito+".json", "w") as write_file:
		json.dump(tablita, write_file)
...
def mosaic(img_path, tiles_path):
	tp = TileProcessor(tiles_path)
	tiles_data = tp.get_tiles()
	tiles_pathsitos = tp.tiles_paths
	with open("paths.json", "w") as write_file:
		json.dump(tiles_pathsitos, write_file)
	for wea in tiles_pathsitos:
		image_data = TargetImage(wea).get_data()
		flp = wea.split("/")[-1]
		compose(image_data, tiles_data,flp)
